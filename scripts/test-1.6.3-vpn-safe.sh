...
grep -q '_ensure_comma_before_marker' "$SCRIPT" || fail "must repair missing comma before singbox torrent block"
grep -q 'nft_dump=' "$SCRIPT" || fail "doctor must snapshot nft (pipefail+grep -q fake-missing ipv6-icmp)"
