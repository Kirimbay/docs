...
[[ "${ver}" == "1.5.0" ]] || { echo "version mismatch: ${ver}"; exit 1; }
...
echo "${doc_out}" | grep -q '1.5.0' || { echo "doctor missed version"; exit 1; }
...
echo "${preview}" | grep -q '80' || { echo "nft preview missing web ports"; exit 1; }
